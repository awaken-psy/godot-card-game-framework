...
func test_anchor_positioning_and_groups():
	for container in get_tree().get_nodes_in_group("card_containers"):
		container.re_place()
	assert_almost_eq(deck.position, Vector2(0, 720), Vector2(5,5),
	...
	assert_almost_eq(discard.position, Vector2(1695, 720), Vector2(5,5),
	...
	await yield_for(0.1)
	assert_almost_eq(hand.position, Vector2(228, 900), Vector2(15,10),
			"Bottom middle anchor placement works")
	assert_almost_eq(hand.control.size, Vector2(1455, 360), Vector2(10,10),
			"Hand correct size not to overlap with piles")
	assert_almost_eq(hand.get_node("CollisionShape2D").shape.extents,
			Vector2(732, 180), Vector2(10,10),
	...
	deck2.re_place()
	assert_almost_eq(deck2.position, Vector2(1690.5, 0), Vector2(5,5),
	...
	deck2.re_place()
	assert_almost_eq(deck2.position, Vector2(843, 0), Vector2(5,5),
			"Top Middle anchor placement works")
	assert_true("top" in deck2.get_groups(), "top Position group assigned")
	deck2.placement = CardContainer.Anchors.RIGHT_MIDDLE
	deck2.re_place()
	assert_almost_eq(deck2.position, Vector2(1690.5, 355.5), Vector2(5,5),
			"Right Middle anchor placement works")
	assert_true("right" in deck2.get_groups(), "right Position group assigned")
	deck2.placement = CardContainer.Anchors.LEFT_MIDDLE
	deck2.re_place()
	assert_almost_eq(deck2.position, Vector2(0, 355.5), Vector2(5,5),
...
func test_overlap_shift_up():
	...
	for container in get_tree().get_nodes_in_group("card_containers"):
		container.re_place()
	await yield_for(0.1)
	assert_almost_eq(deck2.position, Vector2(0, 340), Vector2(10,10),
...
func test_overlap_shift_left():
	...
	for container in get_tree().get_nodes_in_group("card_containers"):
		container.re_place()
	await yield_for(0.1)
	assert_almost_eq(deck2.position, Vector2(1465.5, 720), Vector2(5,5),
	...
	assert_almost_eq(hand.control.size, Vector2(1233, 360), Vector2(10,10),
			"Hand shrunk when overlap was displaced left")
	assert_almost_eq(hand.get_node("CollisionShape2D").shape.extents,
			Vector2(609.38, 180), Vector2(5,5),
...
func test_overlap_shift_right():
	...
	for container in get_tree().get_nodes_in_group("card_containers"):
		container.re_place()
	await yield_for(0.1)
	assert_almost_eq(deck2.position, Vector2(240.75, 720), Vector2(5,5),
			"overlapping deck2 shifted left")
	assert_almost_eq(deck.position, Vector2(0, 720), Vector2(5,5),
			"OverlapShiftDirection == NONE allowed the discard to stay where it is")
	assert_almost_eq(hand.position, Vector2(459, 900), Vector2(5,5),
			"Hand didn't shift when right containers shifted")
	assert_almost_eq(hand.control.size, Vector2(1233, 360), Vector2(10,10),
			"Hand shrunk when overlap was displaced left")
	assert_almost_eq(hand.get_node("CollisionShape2D").shape.extents,
