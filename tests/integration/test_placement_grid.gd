...
func before_each():
	...
	grid.position = Vector2(300, 300)
	grid.visible = true

func test_placement_slots():
	var card = cards[1]
	await drag_card(card, Vector2(375, 450))
	...
	await move_mouse(Vector2(525, 450))
	...
	await drag_drop(card,Vector2(750, 450))
	...
	await drag_drop(card,Vector2(1500, 450))
	...
	await drag_drop(card,Vector2(750, 450))
	await drag_drop(card,Vector2(750, 900))
	assert_null(card._placement_slot,
...
func test_any_grid_placement():
	...
	await move_mouse(Vector2(450, 300))
	await drag_drop(card,Vector2(750, 450))
...
func test_occupied_slot():
	...
	card = cards[3]
	card.board_placement = Card.BoardPlacement.ANY_GRID
	await drag_card(card, Vector2(750, 450))
...
func test_move_script_placed_card_out_of_grid():
	...
	await drag_drop(card,Vector2(1500, 0))
...
func test_grid_auto_placement():
	...
	await drag_drop(cards[1],Vector2(1500, 0))
	await move_mouse(Vector2(450, 300))
	await drag_drop(cards[2],Vector2(1500, 0))
	await move_mouse(Vector2(450, 300))
	await drag_drop(cards[3],Vector2(1500, 0))
	await move_mouse(Vector2(450, 300))
	await drag_drop(cards[4],Vector2(1500, 0))
