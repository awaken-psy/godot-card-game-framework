...
func _set_notice(text: String, colour := Color(0,1,0)) -> void:
	...
	# Godot 4: Tween is no longer a node fetched via get_node("Tween");
	# it is created on demand and auto-starts.
	var tween := create_tween()
	tween.tween_property(_notice, 'modulate:a', 0, 2)\
			.from(1).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_IN)
